# Backend testing script for K8S
...
# Set "url" variable to what's inside the k8s_url.txt file
try:
  with open('k8s_url.txt', 'r') as f:
      url = f.read()
except:
  print("Error with file k8s_url.txt")

# Post a new user data to the REST API using POST method with variables from above, and save response to "res" variable
res = requests.post(url+'/users/'+str(user_id), json={"user_name": user_name})
